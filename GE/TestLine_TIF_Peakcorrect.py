...
def Gaussian_FWHM(x,y,center=1200,title=''):
    ...
    result = gmodel.fit(y, x=x, amp=2048540, cen=center, wid=2.6)
    print(result.values)
    ...
    plt.title(f"Gaussian fit-{title}")
...
def shift_pixel(index:int,j:int=1):
    ...
    return round(-(20/57.0+0.001*j+j**2*(1e-7))*index)  # for test line y=57*x-57000
...
for i in range(10):
    ...
    FWHM,FWHM_err=Gaussian_FWHM(x_correct,y_correct,center=p_col,title=f'{i}')
